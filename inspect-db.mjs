import pg from "pg";
import { getDatabaseUrl } from "./lib/db/get-db-url.mjs";
...
async function run() {
  ...
  const txRes = await client.query(`
    SELECT id, title, amount, type, category, date, account_id, plaid_transaction_id, source
    FROM transactions
  ...
  duplicates.forEach((group, idx) => {
    ...
    group.forEach(tx => {
      ...
      console.log(`  - Tx ID: ${tx.id} | Acct: ${acct ? acct.name : tx.account_id} (${acct ? acct.type : '?'}) | Plaid Tx ID: ${tx.plaid_transaction_id} | Source: ${tx.source}`);
    });
  });
